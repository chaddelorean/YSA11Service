...
YSA11Service.controller('photoUploadToolCtrl', function($scope, $http) {
	  //============== DRAG & DROP =============
    // source for drag&drop: http://www.webappers.com/2011/09/28/drag-drop-file-upload-with-html5-javascript/
    var dropbox = document.getElementById("dropbox");
    $scope.dropText = 'or drop a picture here...';
    ...
    dropbox.addEventListener("dragover", function(evt) {
        ...
        $scope.$apply(function(){
            $scope.dropText = ok ? 'Drop pictures here...' : 'Only pictures are allowed!';
    ...
    dropbox.addEventListener("drop", function(evt) {
        ...
        evt.preventDefault();
        $scope.$apply(function(){
            $scope.dropText = 'Drop pictures here...';
            $scope.dropClass = '';
        })
    ...
    $scope.uploadFile = function() {
        ...
        // }
        var dataArray = [];
        dataArray.push({'password': $scope.password});
        for (var i in $scope.files) {
        	dataArray.push($scope.files[i]);
        }
        ...
        xhr.open("POST", "uploadPics.php");
        xhr.setRequestHeader("Content-Type", "application/json; charset=utf-8");
        $scope.progressVisible = true;
        xhr.send(JSON.stringify(dataArray));
        
        // $http.post('uploadPics.php', dataArray
